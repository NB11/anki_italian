...
const paddedRank = String(rank).padStart(4, "0");
...
function refreshExampleSentences() {
  ...
  // Set pre-generated audio filename for the current sentence so
  // formatLanguageText can wire the play button to the local MP3.
  window._pregenAudio = `${paddedRank}_s${currentSentence + 1}.mp3`;
...
function formatLanguageText(within = document) {
  within.querySelectorAll(".fr").forEach(function (el) {
    ...
    if (shouldShow) {
      ...
      (typeof window._pregenAudio !== "undefined" ? window._pregenAudio : "");
...
// Italian grammar library not yet available – hide the section silently.
grammarLibrary.style.display = "none";
...
/**
 * Easter egg: Clicking on rank shows the number spelled out in Italian
 */
function numberToItalian(n) {
  if (n === 0) return "zero";
  if (n < 0)   return "meno " + numberToItalian(-n);

  const ones  = ["", "uno", "due", "tre", "quattro", "cinque", "sei",
  ...
  if (n >= 1000) {
    ...
    const rem = n % 1000;
    const prefix = th === 1 ? "mille" : numberToItalian(th) + "mila";
    return prefix + (rem > 0 ? numberToItalian(rem) : "");
  }
  if (n >= 100) {
    const h   = Math.floor(n / 100);
    const rem = n % 100;
    const prefix = h === 1 ? "cento" : ones[h] + "cento";
    return prefix + (rem > 0 ? numberToItalian(rem) : "");
  }
  if (n >= 20) {
    const t   = Math.floor(n / 10);
    const u   = n % 10;
    // Drop trailing vowel of tens before "uno" and "otto"
    const base = (u === 1 || u === 8) ? tens[t].slice(0, -1) : tens[t];
    return base + (u > 0 ? ones[u] : "");
  }
  return ones[n];
}
...
if (rank >= 1) {
  rankElement.onclick = function () {
    ...
    rankElement.innerHTML = `<div class="fr force-audio condensed">${numberToItalian(rank)}</div>`;
...
const wordDataEl = document.querySelector('.word');
if (rankEl && wordDataEl) {
  const rankPadded = String(rankEl.dataset.content).padStart(4, '0');
  const safeLemma = (wordDataEl.dataset.word || '').replace(/[^\p{L}\p{N}_\-]/gu, '_');
  github.href = `https://github.com/NB11/anki_italian/tree/main/cards/${rankPadded}_${safeLemma}.yml`;
